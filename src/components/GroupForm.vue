<template>
  <div class="form">
    <label class="field">{{ t('group.name') }} *<input v-model="name" data-test="name" @keydown.enter="trySubmit" /></label>
    <label class="field">{{ t('group.description') }}<input v-model="description" /></label>
  </div>
</template>
