<template>
  <div class="form">
    <label class="field">{{ t('list.name') }} *<input v-model="name" data-test="name" @keydown.enter="trySubmit" /></label>
    <label class="field">{{ t('list.description') }}<input v-model="description" /></label>
  </div>
</template>
